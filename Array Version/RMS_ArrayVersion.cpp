...
string train[array_size] = {"Allama_Iqbal", "Awami_Express", "Pakistan_Express", "Khyber_Mail"};

// all stations that syatem have built in
string stations[50] = {"Rawalpindi", "Gujrat", "Lahore", "Sahiwal", "Faislabad", "Multan", "Sadiqabad"};

// arrays for stations
string ts1[array_size] = {"Lahore", "Gujrat", "Rawalpindi", "Rawalpindi"};     // for station 1
string ts2[array_size] = {"Sahiwal", "Lahore", "Gujrat", "Lahore"};            // for station 2
string ts3[array_size] = {"Multan", "Multan", "Multan", "Sahiwal"};            // for station 3
string ts4[array_size] = {"Sadiqabad", "Sadiqabad", "Faislabad", "Faislabad"}; // for station 4
...
int ts1_ath[array_size] = {8, 12, 6, 3};    // for station 1 arrival hour
int ts1_atm[array_size] = {00, 00, 00, 25}; // for station 1 arrival minutes
int ts2_ath[array_size] = {10, 14, 9, 7};   // for station 2 arrival hour
int ts2_atm[array_size] = {30, 30, 20, 10}; // for station 2 arrival minutes
int ts3_ath[array_size] = {12, 16, 13, 11}; // for station 3 arrival hour
int ts3_atm[array_size] = {00, 00, 00, 05}; // for station 3 arrival minutes
int ts4_ath[array_size] = {14, 20, 17, 13}; // for station 4 arrival hour
int ts4_atm[array_size] = {00, 00, 00, 00}; // for station 4 arrival minutes

// arrays for trains departure times from stations
int ts1_dth[array_size] = {8, 12, 6, 3};    // for station 1 departure hour
int ts1_dtm[array_size] = {20, 20, 20, 50}; // for station 1 departure minutes
int ts2_dth[array_size] = {10, 14, 9, 7};   // for station 2 departure hour
int ts2_dtm[array_size] = {40, 40, 40, 45}; // for station 2 departure minutes
int ts3_dth[array_size] = {12, 16, 13, 11}; // for station 3 departure hour
int ts3_dtm[array_size] = {10, 10, 30, 20}; // for station 3 departure minutes
int ts4_dth[array_size] = {14, 20, 17, 13}; // for station 4 departure hour
int ts4_dtm[array_size] = {30, 30, 40, 30}; // for station 4 departure minutes

// array for prices of trains tickets
int tticket[array_size] = {400, 450, 350, 400};
// array for prices of trains freight rate
int tcargo[array_size] = {45, 50, 40, 45};

// t means tickets buy book means cargo booked
int t = 0, book = 0;

// tickets array user can only buy four tickets
const int ticket_a = 4;
...
int booking_no[ticket_a];     // for the num,ber of booking
            ...
            while (true)
            {
                ...
                else if (option == "4")
                {
                    ...
                    if (t == 5)
                    {
                        ...
                        cout << "_____________________________________________________________" << endl;
                        cout << endl;
                        cout << "You cannot buy more than four tickets ! " << endl;
                        cout << "Press any key for continue....";
                        getch();
                        cout << endl;
                    }
                ...
                else if (option == "7")
                {
                    ...
                    if (book == 5)
                    {
                        ...
                        cout << "_____________________________________________________________" << endl;
                        cout << endl;
                        cout << "You cannot book cargo  more than four times ! " << endl;
                        cout << "Press any key for continue....";
                        getch();
                        cout << endl;
                    }
...
string login_page(void)
{
    while (true) // loop run until user enter wrong value
    {
        ...
        if (login == "1") // if user press 1
        {
            ...
            cin >> password;
            if (password == "123") // if password is correct
            {
                return "admin";
            }
...
int list_of_trains(string name, string title)
{
    ...
    while (true) // loop run until user eneter correct value
    {
        ...
        if (sub_op > a - 1) // if user enter wrong value
        {
            cout << "\nInvalid Option !" << endl;
...
void add_train_route(void)
{
    ...
    while (flag != '1') // loop run until user want to add routes
    {
        ...
        while (true) // validation on correcrt ticket price
        {
            ...
            if (tticket[routes] <= 2000) // must be less than 2000
            {
                break;
            }
            cout << " Train Ticket Price Cannot be greater than 2000. " << endl;
        }
        ...
        while (true) // validation on correcrt cargo price
        {
            ...
            if (tcargo[routes] <= 500) // must be less than 500
            {
                break;
            }
            cout << " Price must be less than 500 per kg " << endl;
        }
        ...
        cout << "Press 1 for exit or any other for adding another route : ";
        cin >> flag;
    }
}
...
void add_notice(void)
{
    ...
    cin >> notice; // string varaible for taking  notice as input
...
void buy_ticket(int index)
{
    ...
    while (true) // validation for quantity
    {
        ...
        if (quantity[t] > 12) // quantity cannot be greater than 12
        {
...
void book_cargo(int index)
{
    ...
    while (true) // validation on weight
    {
        ...
        if (weight[book] > 500) // user canot enter more than 500 kg weight
        {
...
void change_train_name(void)
{
    ...
    while (true)
    {
        ...
        if (a == 1) // if all name met all conditions than change name
        {
            break;
        }
    }
...
void change_train_stations(void)
{
    ...
    cout << endl;
    cout << "Train Name : " << train[idx];

    cout << "Old Station 1 Name : " << ts1[idx];
    ...
    cout << "Old Station 2 Name : " << ts2[idx];
